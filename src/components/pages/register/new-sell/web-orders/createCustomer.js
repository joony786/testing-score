...
const CreateCustomerModal = (props) => {
  ...
  // const [firstNameChar, setFirstNameChar] = useState(false);
  ...
  //   if (value.length < 2) {
  //     //imp
  //     setFirstNameChar(true)
  //   } else {
  //     setFirstNameChar(false);
  //   }
  // };
  const onCreateCustomer = async (event) => {
    ...
    Object.entries(formData).forEach(([key, val]) => {
      ...
      if (key === "first" || key === "last") {
        if (val.length < 2) {
          setErrorMessage("First and Last name must be 2 characters");
          setShowError(true);
          formValidationsPassedCheck = false;
          setTimeout(() => {
            setErrorMessage("");
