...
function EditParentCategory(props) {
  ...
  const { match = {} } = props;
  const { cat_id = {}, parentId = {} } = match !== undefined && match.params;

  let mounted = true;

  useEffect(() => {
    window.scrollTo(0, 0);
    if(parentId !== undefined && cat_id !== undefined){
      fetchParentApisData(parentId, cat_id);
    } else {
      return popPage();
    }
  ...
  const fetchParentApisData = async (parentId, cat_id) => {
    document.getElementById("app-loader-container").style.display = "block";
    await Promise.all([
      getParentCategory(parentId, cat_id),
      //fetchCategoriesData()
    ]);
    document.getElementById("app-loader-container").style.display = "none";
  };

  const getParentCategory = async (parentCategoryId, cat_id) => {
    const getCategoryResponse = await CategoriesApiUtil.getCategory(parentCategoryId);
    if (getCategoryResponse.hasError) {
      ...
      if (mounted) {
        ...
        if(typeof childCategoryExist !== "undefined"){
          childCategoryExist.forEach(item => {
            if(item.id === cat_id){
              setFormData({
              ...
              let catSearchResult =  [...categoriesSearchResult];
              catSearchResult.push({ id: categoryData.id, name: categoryData.name });
              setCategoriesSearchResult(catSearchResult);
