...
import StockNestedVariantsProductsTable from "../../../organism/table/stock/stockNestedVariantsProductsTable";
import "../style.scss";
import {
  returnScanType,
  SearchDataWithoutScan,
} from "../order_stock/order_stock_utlis";
...
const AddOrderQunatityInGRN = ({ grnData, senBackToParent, restData }) => {
  ...
  const renderModalContent = () => {
    return (
      <StockNestedVariantsProductsTable
  ...
  const search = (value, s) => {
    if (s) {
      return grnData.filter((entry) => {
        let searchValue = entry.sku;
  ...
  const handleEnterInputParentSku = (event) => {
    //console.log(event);

    if (event.key === "Enter") {
      //   console.log(" enter");
      //   console.log(scanBoxparentSku, "scane ==="); //imp new searched value
      //   const fetchedBySku = await ProductsApiUtil.fetchProductBySku(
      //     scanBoxparentSku,
      //     20,
      //     1
      //   );
      ...
      const data = search(scanBoxparentSku, "v");
      console.log(data);
  ...
  const handleAddProductData = (selectedProdId) => {
    ...
    let productExistsCheck = false;
    const index = grnData.findIndex(
      (item) => selectedProdId === item.product_id
    );
    if (index > -1) {
      // const selectedItem = grnData[index]
      const selectedItem = JSON.parse(JSON.stringify(grnData[index]));
      productsTableData.forEach((p) => {
      ...
      if (productExistsCheck) {
        const qty = calculateProductsTotalQuantity([...productsTableData]); //imp
        setProductsTableData([...productsTableData]);
        senBackToParent([...productsTableData], qty); //imp
      }
      if (!productExistsCheck) {
        let inputQtyValue = Helpers.var_check(productQty) ? productQty : 1;
        ...
        const qty = calculateProductsTotalQuantity(productsTableData);
        senBackToParent([...productsTableData], qty);
  ...
  const handleAddSkuData = (selectedProdId) => {
    ...
    let productExistsCheck = false;
    const index = grnData.findIndex((item) => selectedProdId === item.sku);
    if (index > -1) {
      // const selectedItem = grnData[index]
      const selectedItem = JSON.parse(JSON.stringify(grnData[index]));
      productsTableData.forEach((p) => {
        console.log(p, "found p inside foreach");
        if (p.sku.toLowerCase() === selectedItem.sku.toLowerCase()) {
          productExistsCheck = true;
          let inputQtyValue = 1;
          p.qty = parseFloat(p.qty) + parseFloat(inputQtyValue);
          console.log(p.qty);
      ...
      if (productExistsCheck) {
        ...
        setProductsTableData([...productsTableData]);
        senBackToParent([...productsTableData], productsTotalQuantity); //imp
      }
      if (!productExistsCheck) {
        let inputQtyValue = Helpers.var_check(productQty) ? productQty : 1;
        selectedItem.qty = parseFloat(inputQtyValue);
        ...
        productsTableData.push(selectedItem);
        calculateProductsTotalQuantity(productsTableData);
        senBackToParent([...productsTableData], productsTotalQuantity);
        //setProductsTableData(newData);  //prev
      }
    }
    if (selectedProdId.id) {
  ...
  const calculateProductsTotalQuantity = (data) => {
    ...
    const newData = [...data];
    //    const newQty= newData.reduce((prev,curr)=>{
...
export default AddOrderQunatityInGRN;
